// Regasification node component for the React Flow canvas.
...
export function RegasNode({ data, selected }: NodeProps<NodeData>) {
  const nodeColor = "#4fd1c5"
  const isOnPath = data.isOptimalPath
  const borderColor = isOnPath ? "#4fd1c5" : (selected ? "rgba(99,179,237,0.6)" : "rgba(255,255,255,0.1)")
  const bgColor = isOnPath ? `rgba(79,209,197, 0.12)` : "rgba(22,28,46,0.9)"
