// Gas Source node component for the React Flow canvas.
...
export function GasSourceNode({ data, selected }: NodeProps<NodeData>) {
  const nodeColor = "#f6c90e"
  const isOnPath = data.isOptimalPath
  const borderColor = isOnPath ? "#f6c90e" : (selected ? "rgba(99,179,237,0.6)" : "rgba(255,255,255,0.1)")
  const bgColor = isOnPath ? `rgba(246,201,14, 0.12)` : "rgba(22,28,46,0.9)"
