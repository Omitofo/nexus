// Liquefaction node component for the React Flow canvas.
...
export function LiquefactionNode({ data, selected }: NodeProps<NodeData>) {
  const nodeColor = "#9f7aea"
  const isOnPath = data.isOptimalPath
  const borderColor = isOnPath ? "#9f7aea" : (selected ? "rgba(99,179,237,0.6)" : "rgba(255,255,255,0.1)")
  const bgColor = isOnPath ? `rgba(159,122,234, 0.12)` : "rgba(22,28,46,0.9)"
